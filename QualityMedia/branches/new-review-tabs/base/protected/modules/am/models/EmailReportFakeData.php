 ...
 */
class EmailReportFakeData extends ActiveRecord
{
    /**
     ...
     * public $dateRange Date range of data (Not implemented yet, as there's no data for monthly searching yet)
     ...
     */
    public function formatEmailContent($emailContent, $photo = null, $graphs = null, $id= null, $today = null)
    {
        ...
        if (isset($graphs) && !empty($graphs)) {
            foreach ($graphs as $replacement) {
                ...
                $emailContent = preg_replace('/<img\sstyle.*' . $replacement . '.*\"\>/',
                    ...
                    getResource("emailReport/images/clientCharts/fakeData/$replacement.png", 's3')
     ...
     */
    public function createAndUploadCharts($s3Component, $bucket, $json, $id, $today)
    {
        ...
        $options = '{"width":1000, "height":180, "imageDir":"' . Yii::app()->getBasePath() . '/tmp/emailReport/images/clientCharts/' . $id . '/' . $today . '"}';
        ...
        $path = Yii::app()->getBasePath() . "/tmp/emailReport/images/clientCharts/$id/$today";
        ...
        foreach ($charts as $chart) {
            ...
            $uri = "emailReport/images/clientCharts/$id/$today/$chart";
    ...
    public function handleS3($yelpAnalyticsStorage, $id, $today, $startDate, $endDate)
    {
        ...
        if (!in_array("emailReport/images/clientCharts/$id/$today/arpu_this_day.png", array_keys($bucketContents))) {
            $this->createAndUploadCharts($s3Component, $bucket, $json, $id, $today);
            // Delete the local directory after the client charts have been uploaded to S3
            if (file_exists(Yii::app()->getBasePath() . "/tmp/emailReport/images/clientCharts/$id/$today")) {
                $this->deleteLocalCharts(Yii::app()->getBasePath() . "/tmp/emailReport/images/clientCharts/$id/$today");
            }
        }
        return $combinedData;
    }
     ...
     */
    public function sendNewEmailReport($photo, $id, $today)
    {
        // Set the model attributes based on submitted form
        $this->setAttributes($_POST['EmailReportFakeData'], false);
        ...
        if ($this->textOrGraph == 'text') {
            ...
            // Format the email body for sending
            $emailContent = $this->formatEmailContent($this->attributes['emailContentGraph'], $photo, $graphs, $id, $today);
        }
