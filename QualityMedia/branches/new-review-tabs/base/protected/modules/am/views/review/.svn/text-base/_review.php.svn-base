<?php
$icons = array('open', 'waiting', 'approved', 'nonapproved', 'reverted');
...
<?php echo CHtml::tag('tr', array('id'=>'review-'.$data->id, 'class'=>($data->isProcessed() ? 'processed' : 'bold'). ($data->isPrecontract() ? ' warning' : '')), false, false);
    ...
    if(!Yii::app()->getUser()->isSenior()):
     ?>
        <td class="approve-status" style="text-align: center;">
            ...
            <?php if($data->lastPrivateReplyPostingStatus || $data->postingStatus): ?>
    ...
    <td class="add-note" style="text-align: center;">
        ...
        <?php if(Yii::app()->getUser()->isSenior() && ($data->lastPrivateReplyPostingStatus || $data->postingStatus)): ?>
            <div class="statusBar success"></div>
        <?php endif; ?>
    </td>
    ...
    <td>
        <?php
            echo $formatter->formatDate($data->user->billingInfo->contractDate);
        ?>
    </td>
    <td>
        <?php
    ...
    <td>
        <?php
        echo $formatter->formatDate($data->createdAt);
        ?>
    </td>
    <td class="last-action">
        <?php
        echo $data->getLastActionTime();
        echo $data->isFlagRejected() ? ' <span class="label label-important">Flag Reverted</span>' : ''; ?>
    </td>
    <td>
...
<tr class="tb-child" data-id="<?php echo $data->id; ?>">
    <td colspan="8">
            ...
            <div class="well well-small public_msg">
                <div class="more-info clearfix">
                    <div class="info_left">
                        ...
                        <?php if (empty($data->accountManagerId)): ?>
                            <div style="float: right; margin-left: 20px;"><span class="label label-success">Business owner answer</span></div>
                        <?php else: ?>
                            <div style="float: right; margin-left: 20px;"><span class="label label-success">Account Manager answer</span></div>
                        <?php endif; ?>
        ...
        <?php if ($data->isFlagRejected()): ?>
            <div class="row-fluid">
                <div class="span6 pull-right">
                    <div class="label label-important">
                       Flag reverted by Senior AM
                    </div>
                </div>
            </div>
        <?php endif; ?>
            ...
            <div class="row-fluid">
                ...
                <?php if($data->isOpened() || $data->isReplied()): ?>
                    ...
                    <!-- Flag -->
                        <?php $this->renderPartial('actions/flag', array('flagReason' => $data->flagReason, 'flagRejected' => $data->isFlagRejected() ? true : false)) ?>
                <?php endif; ?>
