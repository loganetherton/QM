...
<div id="main-content" class="row-fluid gran-data">
    <div id="ch-content" class="span9">
        <?php
            //Sub tabs for inactive reviews
            if($this->currentSubTab != null) {
                $this->renderPartial('/layouts/tabs/reviewJrSubTabs', array('activeSub'=>$this->currentSubTab, 'id' => $accountManager->id));
            }

            //set approval status sortable attribute depending of the current tab
            $approvalStatusSort = ($this->currentTab == 'flagged' ? 'flagApprovalStatus' : 'approvalStatus');
            ...
            $this->widget('ReviewListView', array(
                ...
                'headers'=>array('Approval', 'Client', 'Contract&nbspDate', 'Review&nbsp;Date', 'Added&nbsp;On', 'Last&nbsp;Action', 'Reviewer', 'Rating', ''),
