...
class Message extends ActiveRecord
{
    ...
    public function rules()
    {
        return array(
            ...
            array('postingStatus', 'numerical', 'integerOnly'=>true),
    ...
    protected function afterSave()
    {
        ...
        //If the approval status has been changed, update related AM activity and parent review totalApprovalStatus column
        if(!$this->getIsNewRecord()) {

            if($this->originalApprovalStatus != $this->approvalStatus) {
                $this->updateRelatedAmActivityStatus(AmActivity::TYPE_PRIVATE_MESSAGE);

                $this->review->updateTotalApprovalStatus();
            }
        }
    }
