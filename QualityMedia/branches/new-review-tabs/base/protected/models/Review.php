...
class Review extends ActiveRecord
{
    ...
    /**
     * Limits search criteria to the reviews considered as "newest"
     * @return object self
     */
    public function newestScope()
    {
        ...
        //Exclude filtered reviews
        $this->notFiltered();

        $criteria = new CDbCriteria;

        $minDate = date('Y-m-d H:i:s', strtotime(sprintf('-%s hours', $this->lastHoursPeriod)));
        $condition = "t.createdAt >= :minDate OR (t.precontract = :precontract)";
    ...
    /**
     * Limits search criteria to the reviews considered as "overdue"
     * @return object self
     */
    public function overdueScope()
    {
        //Limit to opened reviews
        $this->opened();
        ...
        //Exclude older precontract reviews
        $this->excludeOlderPrecontract();

        //Exclude reviews Older than last yours
        $this->excludeLastHoursScope();
     ...
     * @return object self
     */
    public function minStarRatingScope($starRating)
    {
        $criteria = new CDbCriteria;
    ...
    public function recalculatePrecontractReviews($clientId, $contractDate)
    {
        ...
        $criteria->addCondition("filtered = ".self::FILTERED_FALSE);
