...
class AmStatsListView extends CListView
{
    /**
    ...
    public function renderItems()
    {
        ...
        $headerFormat = '<th>%s</th>';
        echo '<table class="oview table table-bordered table-striped table-condensed">'."\n";
        echo '<thead class="sorter"><tr>';

        foreach ($this->headers as $header) {

            if(count($this->sortableAttributes) && array_search($header, $this->sortableAttributes)) {
                $header = $sort->link(array_search($header, $this->sortableAttributes), $header);
            }

            echo sprintf($headerFormat, $header);
        }
