...
function pullAccountFields($form, account, options, pull) {
  ...
  account.companyName = pull.field($form, '.contact_info .company_name', V(R.isNotEmpty));
...
R.buildSubscriptionForm = function(options) {
  ...
  function gotPlan(plan) {
    ...
    if(plan.setupFee) {
      ...
      $form.find('.plan .setup_fee .cost').text('$' + plan.setupFee.dollars());
    }
    else {
    ...
    function updateTotals() {
      ...
      $form.find('.add_ons .add_on').each(function() {
        ...
        if($(this).hasClass('selected')) {
          ...
          $(this).find('.cost').text('$'+cost.dollars()+'/mo');
        }
        else {
          $(this).find('.cost').text('$'+addOn.cost.dollars()+'/mo');
        }
      });

      $form.find('.add_ons_summary .add_on_total').each(function() {
        var addOn = $(this).data('add_on');
        $(this).find('.cost').text('$'+addOn.cost.dollars()+'/mo');
      });
    ...
    var $addOnsSummary = $form.find('.add_ons_summary');
    ...
    if(options.enableAddOns) {
      ...
      if(l) {
        ...
        for(var i=0; i < l; ++i) {
          ...
          var $addOn = $('<div id="add_on_'+addOn.code+'" class="'+classAttr+'">' +
          '<h6 class="name">'+addOn.name+'<span class="addonRemove right"><a href="#">Remove</a></span></h6>' +
          ...
          //addon summary
          var $addOnTotal = $('<p style="display: none" class="add_on_total add_on_total_' + addOn.code + '">' +
          '<span class="label">&nbsp;</span>' +
          '<span class="field"><span class="cost"></span> ('+addOn.name+')</span>' +
          '</p>');
          if(!addOn.displayQuantity) {
            $addOnTotal.find('.quantity').remove();
          }
          $addOnTotal.data('add_on', addOn);
          $addOnTotal.appendTo($addOnsSummary);
        }
        ...
        $addOnsList.delegate('.add_on', 'click', function(e) {
          ...
          var addOn = $(this).data('add_on');
          var totalSelector = $('.add_on_total_'+$(this).attr('id').substr(7));
    ...
    $form.submit(function(e) {
      ...
      validationGroup(function(puller) {
        ...
        //@QM - check email exsistance before seding to recurly
        var $input = $form.find('.email input');
        $.post('/registration/validate', {'email': $input.val()}, function(response) {
            ...
            if(responseData.status == 'success') {
                ...
                displayServerErrors($form, responseData.errors);
            }
