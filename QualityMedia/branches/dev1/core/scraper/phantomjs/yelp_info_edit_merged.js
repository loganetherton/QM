...
function readArguments() {
    ...
    businessInfo.type = system.args[4].split(',');
...
page.onCallback = function (json) {
    if (json.method == "logged_out") {
        ...
        if (businessInfo.type.indexOf(json.info_type) < 0) {
            console.log(constants.editBizInfoIncompleteDataError);
            utils.phantomExit();
        }
        steps.push(redirectStep(json.url));
        steps.push(scrapper[json.info_type](json.info_type));
        steps.push(statusCheck(json.info_type));
        if (json.final_step) {
            steps.push(finalStep);
        }

    } else if (json.method == "add_status") {
        statusArray.push({
...
var statusCheck = function (info_type) {
    return function () {
...
var scrapper = {
    ...
    basic_info: function (info_type) {
        return function () {
            ...
            }, businessInfo.info[info_type], constants);
        }
    },

    //Hours info scrapper
    hours: function (info_type) {
        return function () {
            ...
            }, businessInfo.info[info_type]);
        }
    },

    //specialties info scrapper
    specialties: function (info_type) {
        return function () {
            page.evaluate(function (data) {
...
var steps = [
    ...
    function () {
        ...
        page.evaluate(function () {
			...
			document.location.href = document.querySelector(".biz_site_biz_info  a").href;
        });
    },

    function () {
        //Open first section
        page.evaluate(function (info_type_array) {
            var url;
            for (i = 0; i < info_type_array.length; i++) {
                switch (info_type_array[i]) {
                case "basic_info":
                ...
                if (i + 1 == info_type_array.length) {
                    window.callPhantom({
                        "method": "add_next_step",
                        "url": url,
                        "info_type": info_type_array[i],
                        "final_step": true
                    });
                } else {
                    window.callPhantom({
