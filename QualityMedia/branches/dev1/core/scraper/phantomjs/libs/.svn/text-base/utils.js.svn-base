...
var constants = {
	...
	reviewAlreadyFlaggedError : '{"error": "review_already_flagged"}',
	...
	noBizAccessError : '{"error": "no_biz_access_error"}',
...
var blacklist = {
    ...
    loadBlacklist: function() {
        var blacklist = [];
        var f = fs.open(config.hostBlacklistFile, 'r');
        ...
        if(config.useHostBlacklist) {
            utils.log('Blacklist loaded (' + blacklist.length + ' elements)');
        }
    ...
    isUrlAllowed: function(url) {
        ...
        if(pieces1.length > 1) {
            ...
            for(var i=0; i < blacklist.length; i++) {
                if(this.strEndsWith(host, blacklist[i])) {
                    // Blacklisted.
                    return false;
                }
            }
...
var utils = {
    ...
    checkGenericErrors: function (error) {
        ...
        case "account_not_confirmed":
            ...
            break;
		case "captcha_error":
            console.log(constants.captchaError);
            utils.phantomExit();
            break;
		case "no_biz_access":
            console.log(constants.noBizAccessError);
            utils.phantomExit();
            break;
		default:
            ;
    ...
    log: function (value) {
        ...
        if (that.intialRun) {
            ...
            if(yelpObject){
				...
				if(yelpObject.account['biz_id']){
					file.write(" ,\Biz ID: " + yelpObject.account['biz_id']);
				}
        ...
        if (value.indexOf(constants.unsafeSrciptLog) != -1) {
            file.write("\n[" + that.getCurrentTime() + "] -> "+ yelpObject.account['username'] +" : " + 'Unsafe script');
        } else if (value.indexOf(constants.insecureContentLog) != -1) {
            file.write("\n[" + that.getCurrentTime() + "] -> "+ yelpObject.account['username'] +" : " + 'Insecure content loaded');
        } else {
            file.write("\n[" + that.getCurrentTime() + "] -> "+ yelpObject.account['username'] +" : " + value);
        }
...
var phantomPage = {
    ...
    onResourceRequested: function (requestData, networkRequest) {
        // utils.log('Requested resource: ' + requestData.url);

        if(!blacklist.isUrlAllowed(requestData.url)) {
            // utils.log("    > Denied: " + requestData.method + " to " + requestData.url);
    ...
    checkLoginStatus: function () {
        page.evaluate(function () {
            ...
            if (header && header.innerText.indexOf('Logged in as') > -1) {
				...
				var CAPTCHA_MSG = "Please complete the word verification below to continue";
				var NO_BIZ_ACCESS = "You don't have access to any businesses";
                ...
                } else if(body.innerText.indexOf(CAPTCHA_MSG)>-1){
                    window.callPhantom({
                        "method": "captcha_error"
                    });
                } else if(body.innerText.indexOf(NO_BIZ_ACCESS)>-1){
                    window.callPhantom({
                        "method": "no_biz_access"
                    });
                } else {
                    window.callPhantom({
                        "method": "logged_out"
