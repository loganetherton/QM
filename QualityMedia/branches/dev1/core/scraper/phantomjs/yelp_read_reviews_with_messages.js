...
page.onCallback = function (json) {
    if (json.method == "append_reviews") {
        ...
        utils.log("Skipping in between msg steps.");
    ...
    page.evaluate(function (isFilteredSection) {
        ...
        if (!isFilteredSection) {
            ...
            if (paginationDiv && paginationDiv.innerText.indexOf("Next") > -1) {
                ...
                var filteredLink = document.querySelector('#filtered-reviews-link');;
                if(filteredLink){
                    window.callPhantom({
                        "method": "redirect_to_filtered_reviews",
                        "url": filteredLink.href
                    });
                }
            }
        } else {
    ...
    return function () {
        page.evaluate(function (inFilteredReviews) {
            ...
            for (i = 0; i < reviewsThread.length; i++) {
                ...
                //User info
                //Check to see if its a qype user
                if(reviewsThread[i].querySelector('.user-passport-info .user-name .ghost-qyper')){
                    //Its a qype user
                    _reviewTemplate.user.from_qype = true;
                    _reviewTemplate.user.user_id = "";
                    _reviewTemplate.user.user_name = "";
                    _reviewTemplate.user.user_yelp_profile = "";
                    
                }else{
                    _reviewTemplate.user.from_qype = false;
...
var addMessageScrapperSteps = function () {
    utils.log("PM section start");
    utils.log("Steps length:" + steps.length);
    ...
    //Redirect to correct biz
    steps.push(common.multiBizRedirect(reviewsObject.business_id));
