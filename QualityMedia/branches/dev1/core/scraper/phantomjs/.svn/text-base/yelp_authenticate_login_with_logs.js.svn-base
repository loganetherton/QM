...
page.onCallback = function (json) {
    if (json.method == "logged_out") {
        ...
        utils.log('Adding yelp ids to biz list');
        ...
        utils.log("Biz acc data = " + JSON.stringify(bizAccountData.biz_list));
    }
...
var readYelpId = function (biz_id) {
    return function () {
        page.evaluate(function (biz_id) {
            //Check the id's here
            console.log("Redirecting to biz id :"+biz_id);
            var yelp_id = document.querySelector('.view-biz-page').href.split('/')[this.length-1];
            window.callPhantom({
                "method": "add_yelp_id",
                "yelp_id": yelp_id,
                "biz_id": biz_id
...
function() {
	page.evaluate(function() {
		...
		if (header && header.innerText.indexOf('Logged in as') > -1) {
			...
			if(bizList.length<=0){
				console.log("Only one biz in this account");
                //Only one business
				var bizObject= {
						...
						"id":document.querySelector('#biz-nav li').querySelector('.top-nav-link').href.split('/')[this.length-1],
                        "yelp_id": document.querySelector('.view-biz-page').href.split('/')[this.length-1]
					};
				bizNames.push(bizObject);
			}else{
				console.log("Multiple biz in this acc, size = "+bizList.length);
				...
				for ( i = 0; i < bizList.length; i++) {
					var bizObject= {
						...
						"id":bizList[i].querySelector('h6 a').href.split('/')[this.length-1],
                        "yelp_id": ""
					};
