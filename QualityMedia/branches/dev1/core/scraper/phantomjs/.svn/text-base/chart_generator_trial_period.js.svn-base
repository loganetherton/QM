//PhantomJS script to generate image charts for trial period
/**
 ...
 * @userAgent User agent to be used (Optional)
 ...
 Usage:
 phantomjs chart_generator_trial_period.js '<url>' '<data>' '<options>'
 */
...
function readArguments() {
	...
	charts.startDate = utilFunctions.createDateObject(charts.dateOnlyData['dateStart']);
	charts.endDate = utilFunctions.createDateObject(charts.dateOnlyData['dateEnd']);
...
var dataProcessor = {
	...
	chartDataMaker: function (name, type) {
		...
		if (type === "monthly") {
			...
			var month = (charts.endDate.getMonth()+1).toString();
			var year = charts.endDate.getFullYear().toString();
			
			var startMonth = (charts.startDate.getMonth()+1).toString();
			var startYear = charts.startDate.getFullYear().toString();
			var labelMonth;
					
			if(year!=startYear){
			 labelMonth = (utilFunctions.getMonthNameByIndex(parseInt(startMonth))+ " "+startYear.substr(2, 2) + "-" + utilFunctions.getMonthNameByIndex(parseInt(month))+ " "+year);
			}else{
			 if(month!=startMonth){
				labelMonth = (utilFunctions.getMonthNameByIndex(parseInt(startMonth)) + "-" + utilFunctions.getMonthNameByIndex(parseInt(month))+ " "+year);
			 }else{
				labelMonth = (utilFunctions.getMonthNameByIndex(parseInt(month))+ " "+year);
			 }
			}
			
			//for (var year in datesArray) {
				//for (var month in datesArray[year]) {
					//charts.individualData[name]["labels"].push(utilFunctions.getMonthNameByIndex(parseInt(month)) + " " + year.substr(2, 2));
					charts.individualData[name]["labels"].push(labelMonth);
				...
				for (var month in datesArray[year]) {
					...
					for (var day in datesArray[year][month]) {
						...
						if (refinedJsonData[year][month][datesArray[year][month][day]].hasOwnProperty(name)) {
							...
							if (!charts.individualData[name]["datasets"][datesArray[year][month][day]]) {
								charts.individualData[name]["datasets"][datesArray[year][month][day]] = {}
								charts.individualData[name]["datasets"][datesArray[year][month][day]]["data"] = new Array();
								if(charts.individualData[name]["labels"].length>1){
									charts.individualData[name]["datasets"][datesArray[year][month][day]]["data"].push(null);
								}
							}

							//Additional steps to push null values
							/*if(datesArray[year][month][day]>1 && firstDayFlag){
								for(i=1;i<datesArray[year][month][day];i++){
									
									if (!charts.individualData[name]["datasets"][i]) {
										charts.individualData[name]["datasets"][i] = {};
										charts.individualData[name]["datasets"][i]["data"] = new Array();
									}
									charts.individualData[name]["datasets"][i]["data"].push(null);
								}
							}
							
							firstDayFlag=false;*/
							
							charts.individualData[name]["datasets"][datesArray[year][month][day]]["fillColor"] = "rgba(40,155,253,1)";
							charts.individualData[name]["datasets"][datesArray[year][month][day]]["negativeColor"] = "rgba(255,68,68,1)";
							charts.individualData[name]["datasets"][datesArray[year][month][day]]["data"].push(refinedJsonData[year][month][datesArray[year][month][day]][name]);
