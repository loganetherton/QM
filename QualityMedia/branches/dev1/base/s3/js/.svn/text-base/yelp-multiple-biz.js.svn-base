...
jQuery(document).ready(function() {
    jQuery('#yelp-get-businesses').click(function() {
        var el = jQuery(this);

        var yelpUsername = jQuery('#Profile_yelpUsername').val();
        var yelpPassword = jQuery('#Profile_yelpPassword').val();

        if(yelpUsername == '') {
            alert('Yelp username cannot be blank');
            return false;
        }

        if(yelpPassword == '') {
            alert('Yelp password cannot be blank');
            return false;
        }
        ...
        jQuery.ajax({
            ...
            'complete'  : function() {
                el.html('Get businesses');
                el.removeAttr('disabled');
            },

            'success'   : function(json) {
                var template = jQuery.format(jQuery('#template').html());
                var target = jQuery('#target');

                jQuery.each(json, function(index, biz) {
                    if(jQuery('#yelp-template-'+biz.id).length == 0) {
                        var input = jQuery(template(biz.id));

                        input.find('.templateBizId').val(biz.id);
                        input.find('.templateYelpId').val(biz.yelp_id);
                        input.find('.templateLabel').val(biz.name);
