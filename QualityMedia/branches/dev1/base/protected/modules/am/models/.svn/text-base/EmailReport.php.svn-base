...
class EmailReport extends ActiveRecord
{
    ...
    public function rules()
    {
        return array(
            ...
            array('image_attachment', 'file', 'allowEmpty' => true),
        );
    }
     ...
     * @param int $jrViewId Junior Am id for handling Jr reviewing mode
     * @return string Redirect URL
     */
    public function handleRedirection($id, $docDateRange, $redirectNumber, $startDate, $endDate, $jrViewId = null)
    {
        if (isset($docDateRange['dateRangeStartInvalid'])) {
            ...
            if($jrViewId) {
                $urlParams['jr'] = $jrViewId;
            }

            // Redirect to the first good start date
            return Yii::app()->createUrl('am/emailReport/create', $urlParams);
        }
        if (isset($docDateRange['dateRangeEndInvalid'])) {
            ...
            if (!empty($_GET['startDate'])) {
                if (preg_match('/^\d{8}$/', $_GET['startDate'])) {
                    $startDateFromUri = (int)$_GET['startDate'];
                }
            }

            $urlParams = array(
                'id' => $id,
                'startDate' => isset($startDateFromUri) ? $startDateFromUri : $startDate,
    ...
    public function getClientsForDropdown($client)
    {
        ...
        if ($client->accountManager->showOnlyLinkedFeeds == 1) {
            ...
            if(Yii::app()->user->isSenior()) {
                ...
                $linkedJuniorAms = Yii::app()->user->user->getLinkedAccountManagers();
                $linkedJuniorAms->pagination = false;

                $juniorAmsIds = implode(', ', array_map(function($juniorAm) {return $juniorAm->id; }, $linkedJuniorAms->getData()));

                $yelpClients = Client::model()->findAll("accountManagerId IN({$juniorAmsIds})");
            }
            else {
            ...
            foreach ($yelpClients as $yelpClient) {
                $clientsForDropdown[] = YelpBusiness::model()->findAll('userId = ' . $yelpClient->id . ' AND status=1');
            }
        } else {
            $clientsForDropdown = YelpBusiness::model()->findAll('status=1');
        }
     ...
     * @return YiiMailer
     */
    public function setMailParameters($attachment = null)
    {
        $mail = new YiiMailer();
        if (isset($attachment)) {
            $mail->setAttachment(array($attachment->tempName => $attachment->name));
        }
        $mail->isSmtp();
        $mail->SMTPAuth = true;
        $mail->SMTPSecure = 'tls';
        ...
        $mail->setFrom('no-reply@qualitymedia.com', $this->fromAddress);
        // This is now live, make sure to put in a valid email address if it's a test email
     ...
     */
    public function createAndUploadCharts($s3Component, $bucket, $json, $id, $dateRange, $oneMonth)
    {
        ...
        // Create the directory to write the charts to
        if (!file_exists(Yii::app()->getRuntimePath() . '/charts/' . $id . '/' . $dateRange)) {
            mkdir(Yii::app()->getRuntimePath() . '/charts/' . $id . '/' . $dateRange, 0700, true);
        }

        // We need a writable path to temporarily create the images directory
        $options = '{"width":1000, "height":180, "imageDir":"' . Yii::app()->getRuntimePath() . '/charts/' . $id . '/' . $dateRange . '"}';
     ...
     *
     * @return mixed Data from within the requested date range
     */
    public function handleS3($yelpAnalyticsStorage, $id, $today, $startDate, $endDate)
    {
        ...
        // Determine if we're creating a chart for a single month
        $dateChecker = new DateTime($endDate);
        $dateChecker->sub(new DateInterval('P2M'));
        $oneMonth = $startDate > $dateChecker->format('Ymd') ? true : false;
        // Encode to JSON for JS chart creation
        $json = CJSON::encode($combinedData);
        // Connect to S3
        $s3Component = Yii::app()->s3Component;

        $bucket = Yii::app()->s3Resource->bucket;
...
//        $bucketContents = $s3Component->getBucket($bucket);
        ...
        /**
         * I'm leaving commented so the charts are always created for QA purposes
         */
        //        if (!in_array("emailReport/images/clientCharts/$id/$dateRange/num_ad_clicks.png", array_keys($bucketContents))) {
        $this->createAndUploadCharts($s3Component, $bucket, $json, $id, $dateRange, $oneMonth);
            // Delete the local directory after the client charts have been uploaded to S3
            if (file_exists(Yii::app()->getRuntimePath() . "/charts/$id/$dateRange")) {
                $this->deleteLocalCharts(Yii::app()->getRuntimePath() . "/charts/$id/$dateRange");
            }
//        }
        return $combinedData;
    }
     ...
     */
    public function sendNewEmailReport($photo, $id, $startDate, $endDate, $attachment = null, $attachmentUri = null)
    {
        ...
        if (isset($attachmentUri)) {
            $this->image_attachment = $attachmentUri;
        }

        // Save the report and send it
        if ($this->save()) {
            // Set the standard mail parameters
            $mail = $this->setMailParameters($attachment);
            // Send the email
