...
class ReviewAnswerController extends AmController
{
    ...
    protected function flagChange()
    {
        ...
        //flag rejecting
        $flagReject = isset($_POST['Review']['action']['flagReject']);
        $actionName = sprintf('Flag has been %s', $flagReject ? 'reverted' : 'updated');

        if ($this->model->markAsFlagged($_POST['Review']['flagReason'], $_POST['Review']['flagCategory'], true, true, null, $flagReject)) {
            Yii::app()->getUser()->setFlash('success', $actionName);
        }
        else {
            Yii::app()->getUser()->setFlash('error', $actionName);
        }
    }
}