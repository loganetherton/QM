...
try {
    ...
    $clientsForDropdown = EmailReport::formatClientsArrayForDropdown($clientsForDropdown);
    ...
    foreach ($clientsForDropdown as $clientDrop) {
        ...
        $yelpBusinessId = $clientDrop[0]->id;
            $clientNames['items'][] = array('label' => $clientDrop[0]->label,
                                            'url'   => $this->createUrl('emailReport/create/id/' . $yelpBusinessId.($this->jrViewId ? "/jr/{$this->jrViewId}" : '')));
    }

    // Format client label for display
    if (strpos($yelpBusiness->label, '(') !== FALSE) {
        $remove = !empty($errorMessage) ?: strpos($yelpBusiness->label, ' (');
        $clientLabel = !empty($errorMessage) ?: substr($yelpBusiness->label, 0, $remove);
        $address = !empty($errorMessage) ?: substr($yelpBusiness->label, $remove + 2, -1);
    } else {
        $clientLabel = $yelpBusiness->label;
        $address = '';
    }
    ...
    <div class='lst-table e-report child-content well' style='margin-bottom: 0px; position: relative; min-height: 500px;'>
            ...
            <table style="width:100%" cellpadding="">
                <tr>
                    <td style="width:55%;color:rgb(128, 128, 128);">
                        ...
                        <h4 style='color: rgb(52,52,52);'><?php echo empty($errorMessage) ? $address : ''; ?></h4>
                    </td>
        ...
        <?php
            ...
            $form = $this->beginWidget('bootstrap.widgets.TbActiveForm', array(
                'method' => 'post',
                'action' => $this->createUrl('emailReport/create/id/' . $id . $startDateUrl . $endDateUrl.($this->jrViewId ? "/jr/{$this->jrViewId}" : '')),
            ...
            <table style="font-size: 1.2em; margin-left: 1%">
                <tr>
                    <td style="width:255px;margin-top: 0px">
                        <input type="radio" name='dateRangeRadio' value="lastThirtyDays" checked='checked' disabled="<?php isset($oneMonth) ? 'disabled':false?>"></input>
                       <?php echo isset($oneMonth) ? 'Previous month' : 'Previous 2 months';?>
                    </td>
                    <td>
                        <input type="radio" name='dateRangeRadio' value="customRange" disabled="<?php isset($oneMonth) ? 'disabled':false?>"></input>
        ...
        <div class='row-fluid' style='padding-bottom: 1em'>
            ...
            <div id='datePicker' style='padding: 5px 0 10px 0; width: 100%'>
                <div style="width: 15%; height: auto; position: absolute; margin-left: 20%;">
                    ...
                    echo $form->datepickerRow(
                        ...
                        array(
                            ...
                            'class' => 'input-append date dateEnd',
                            'value' => $endDateFormattedDatePicker,
                            'format' => 'yyyymmdd',
                            'disabled' => isset($oneMonth) ? 'disabled':false,
                        )
                    );
                    ?>
                </div>
                <div style="width: 15%; height: auto; position: relative; margin-left: 1%">
                    ...
                    echo $form->datepickerRow(
                        ...
                        array(
                            'options' => array('language' => 'en'),
                            'prepend' => '<i class="icon-calendar"></i>',
                            'class'   => 'input-append date dateStart',
                ...
                <?php if (!isset($oneMonth)):?>
                <div style='margin-left: 3%' id='updateDateRange'><a href='
                    <?php
            ...
            <table style="font-size: 1.2em; margin-left: 1%;">
                <tr>
                    <td style="width:255px;">
                        <input type="radio" id='displayGraphs' name='EmailReport[textOrGraph]' value="graph" checked='checked'></input>
                        Graphs
                    </td>
                    <td>
                        <input type="radio" id='displayText' name='EmailReport[textOrGraph]' value="text"></input>
                       Text values
                    </td>
                </tr>
            </table>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls" style='border: inset rgb(152,152,152); padding-bottom:10px'>
                            <div class='row-fluid'
                                ...
                                <?php
                                    $emailContentText = $this->renderPartial('/emailReport/_contentText', array(
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls" style='border: inset rgb(152,152,152); padding-bottom:10px'>
                            <div class='row-fluid'
                                 style="width: 99%; height: 1000px; vertical-align: middle; cursor: text; max-width: inherit">
                                <?php
                                    $emailContentGraph = $this->renderPartial('/emailReport/_contentGraph', array(
                                        'clientLabel' => $clientLabel,
                                        'id' => $id,
                                        'startDate' => $startDateFormatted,
                                        'endDate' => $endDateFormatted,
                ...
                <div class='row-fluid span12'>
                    <div class="control-group emailVerticalAlign">
                        <div class="controls">
                            ...
                            // Alow the user to upload images
                            echo $form->fileFieldRow($model, 'image_attachment');
