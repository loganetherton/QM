...
$singleMonthChecker = clone $dateTwo;
$monthTwo = $dateTwo->format('Ym');
$dateTwo = $dateTwo->format('M Y');


$singleMonthChecker->modify('first day of last month');
// Check to see if we have data for two distinct months
if ($dateOne == $singleMonthChecker->format('M Y')) {
    $dateOne = $dateTwo;
    $delta = '--';
...
<tr>
    ...
    <td style="border-top: 1px solid #fff;border-bottom: 1px solid #fff;width:15%;background:#D7EBF4;padding:5px 10px;vertical-align:middle;text-align:right;color:#289BFD;">
        <?php if (is_numeric($delta) && $delta >= 0) {echo CHtml::image($this->resourceUrl("emailReport/images/arrow-up.png", 's3'));} ?>
        <?php echo $delta;?>
    </td>
    <?php } ?>
</tr>
