...
//Get Linked junior managers
$linkedJuniorManagers = $accountManagerModel->findBySeniorManagerId($model->id);
...
<h2><i class="icon3-user"></i> <?php echo $legend; ?></h2>
    ...
    <div>
        <?php echo $form->dropDownList($model, 'type', $model->getTypes()); ?> Account Manager Type
    </div>
    <div id="seniorAmSelect"<?php echo $model->type != AccountManager::TYPE_JUNIOR ? ' style="display: none"':'' ?>>
                ...
                $(document).ready(function() {
                    ...
                    linkSubmitBtn.click(function(e) {
                        ...
                        $.ajax({
                            ...
                            data: {'ids':ids, 'seniorManagerId': ".$model->id."},
                            dataType: 'json',
                 ...
                 $this->widget('bootstrap.widgets.TbGridView', array(
                    ...
                    'columns'=>array(
                        ...
                        array(
                            'name'=>'enabled',
                            'type'=>'raw',
                            'value'=>'CHtml::link("Remove Link", array("ajaxUnlinkAccountManager", "id"=>$data->id, "mode"=>"update"), array("class"=>"unlinkAm"));',
                        ),
                    ),
                ));
                ?>
...
//Give a possible JR Am's unlink warning if the Senior AM is changed Junior horizontalForm

if($model->type != AccountManager::TYPE_JUNIOR) {
    Yii::app()->clientScript->registerScript('TypeChangeWarning', "

    $('#horizontalForm').submit(function(e) {
        var amTypeSelect = $('#CreateAccountManagerForm_type');

        if(amTypeSelect.val() == ".AccountManager::TYPE_JUNIOR.") {
            e.preventDefault();
            var typeChoice = confirm('Are you sure you want to change the AM type to Junior?\\nThis will unlink all current Junior Managers.\\nThis operation is irreversible:');

            if(typeChoice == true) {
                $('#horizontalForm').unbind().submit();
            }
        }
    });
