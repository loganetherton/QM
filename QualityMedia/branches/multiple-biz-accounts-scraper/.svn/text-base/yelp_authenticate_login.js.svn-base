...
var readYelpId = function (biz_id) {
    return function () {
        page.evaluate(function (biz_id) {
            //Check the id's here
            var yelp_id = document.querySelector('.view-biz-page').href.split('/')[this.length];
            window.callPhantom({
                "method": "add_yelp_id",
                "yelp_id": yelp_id,
                "biz_id": biz_id
		...
		if (header && header.innerText.indexOf('Logged in as') > -1) {
			...
			if(bizList.length<=0){
				...
				var bizObject= {
						...
						"id":document.querySelector('#biz-nav li').querySelector('.top-nav-link').href.split('/')[this.length],
                        "yelp_id": document.querySelector('.view-biz-page').href.split('/')[this.length]
					};
				...
				for ( i = 0; i < bizList.length; i++) {
					var bizObject= {
						...
						"id":bizList[i].querySelector('h6 a').href.split('/')[this.length],
                        "yelp_id": ""
					};
