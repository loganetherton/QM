...
class Controller extends CController
{
    ...
    protected function beforeAction($action)
    {
        ...
        if(Yii::app()->params['forceSSL']) {

            //ssl redirect for non ssl request for signup domain
            $signupDomain = substr(Yii::app()->params['domains']['signup'], 8);

            if ($_SERVER['HTTP_HOST'] == $signupDomain && (!isset($_SERVER['HTTPS']) || $_SERVER['HTTPS'] != 'on')) {
                header ('HTTP/1.1 302 Found');
                header ('Location: ' . Yii::app()->params['domains']['signup'] . $_SERVER['REQUEST_URI']);
            }
