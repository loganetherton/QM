...
class YelpTotalReviewReceiver extends CFormModel
{
    ...
    /**
     * @var int Total number of reviews to be considered pre contract
     */
    const PRECONTRACT_COUNT = 10;
        ...
        foreach($values['reviews'] as $review) {
            ...
            $attributes = array(
                ...
                'review'=>array(
                    ...
                    'flagApprovalStatus'=> (bool)$review['isFlagged'] ? Review::APPROVAL_STATUS_ACCEPTED : Review::APPROVAL_STATUS_OPEN,
    ...
    public function save()
    {
        ...
        try {
            ...
            foreach($this->reviews as $review) {
                ...
                // This is a precontract case?
                if (strtotime($reviewModel->reviewDate) < $contractDate) {
                    if ($precontract_count < self::PRECONTRACT_COUNT) {
                        $precontract_count++;
                        $reviewModel->precontract = Review::PRECONTRACT_TRUE;
                    }
                    else {
                        $reviewModel->precontract = Review::PRECONTRACT_TRUE_OLD;
                    }
                }
                else {
                    $reviewModel->precontract = Review::PRECONTRACT_FALSE;
                }
