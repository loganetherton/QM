...
phantom.injectJs("libs/exp_utils.js");
...
var currentIndex=0,startTimeCI,currentTime;
//Interval to execute steps
var interval = setInterval(function () {
    
    //Debug logger(initial test 2)
    if(stepIndex>currentIndex){
        currentIndex = stepIndex;
        startTimeCI = new Date().getTime()/1000;
    }else{
        currentTime = new Date().getTime()/1000;
        if(currentTime-startTimeCI>300){
           utils.checkGenericErrors("script_freeze"); 
        }
        
    }
    //Debug logger(initial test 2 ends)
