...
class ReviewAnswerController extends AmController
{
    ...
    protected function answerWithPublicMessage()
    {
        ...
        if($this->model->answerWithPublicComment($comment, $author)) {
            Yii::app()->getUser()->setFlash('success', sprintf('A public message has been posted', $this->model->userName));
        }
        else {
    ...
    protected function flagReview()
    {
        ...
        if($this->model->markAsFlagged($_POST['Review']['flagReason'], $_POST['Review']['flagCategory'])) {
